...
public class H5QualUpDownConcTimeSliceServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			response.getWriter().println("\"startTime\": \"" + startTime.dateAndTime(104) + "\",");
			response.getWriter().println("\"endTime\": \"" + endTime.dateAndTime(104) + "\",");
			...
			for (int j = 0; j < sliceSize; j++) {
				...
				response.getWriter().print("[");
				for (int i = 0; i < channelArray.length; i++) {
					response.getWriter().print(fData1[j * channelArray.length + i]);
					if (i == channelArray.length - 1)
						break;
					response.getWriter().print(",");
				}
				...
				response.getWriter().print("[");
				for (int i = 0; i < channelArray.length; i++) {
					response.getWriter().print(fData2[j * channelArray.length + i]);
					if (i == channelArray.length - 1)
						break;
					response.getWriter().print(",");
				}
