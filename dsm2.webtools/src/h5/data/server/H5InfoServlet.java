 ...
 * on the file.
 */
public class H5InfoServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public H5InfoServlet() {
		super();
	}
	...
	public void addPathToTableMap(Group g, HashMap<String, Object> map) {
		for (Object member : g.getMemberList()) {
			if (member instanceof Group) {
				addPathToTableMap((Group) member, map);
			} else if (member instanceof ScalarDS) {
				map.put(((ScalarDS) member).getFullName(), member);
			} else if (member instanceof CompoundDS) {
				map.put(((CompoundDS) member).getFullName(), member);
			} else if (member instanceof Datatype) {
				map.put(((Datatype) member).getFullName(), member);
			} else {
				System.err.println("Unknown type of member: " + member);
			}
		}
	}

	/**
	 * builds a map of paths to tables for the h5 file.
	 * 
	 * @param h5file
	 * @return
	 * @throws Exception
	 */
	public HashMap<String, Object> buildPathToTableMap(H5File h5file) throws Exception {
		HObject hObject = h5file.get("/");
		HashMap<String, Object> map = new HashMap<String, Object>();
		if (hObject != null && hObject instanceof Group) {
			Group g = (Group) hObject;
			addPathToTableMap(g, map);
		}
		return map;
	}
	...
	public H5FileInfo extractFileInfo(String file) throws Exception, HDF5Exception, OutOfMemoryError {
		...
		h5file.open();
		HashMap<String,Object> pathToTableMap = buildPathToTableMap(h5file);
		...
		if (inputTables != null) {
			...
			for (int i = 0; i < inputTables.getNumberOfMembersInFile(); i++) {
				...
				if (model.equals("qual") && table.name.equals("output_channel_source_track")) {
					// constituent names either in constituent_names table or derived from looping
					// over unique variable.source_group columns from output_channel_source_track
					if (table.count != 0) {
		...
		for (int i = 0; i < fileInfo.dataTypeNames.length; i++) {
			...
			for (int l = 0; l < layers.length; l++) {
				...
				for (int z = 0; z < zones.length; z++) {
					...
					dataTypeNames[(i * layers.length + l) * zones.length + z] = typeName + "x" + layerName + "x"
							+ zoneName;
				}
			}
		}
