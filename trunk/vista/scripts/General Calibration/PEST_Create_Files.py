import re, os, glob, datetime, random, math, shutil
...
#                     ,['DRAIN-EC' ,0.1] \
    ...
    try: shutil.copyfile(CommonDir + ChanInpFile, PESTDir + ChanCalibFile)
    except:
...
#             pT = perType
    ...
    if param in paramGroups:
        for rowNo in range(len(HydroAgDict['NAME'])):
            try: CPart = HydroAgDict['PATH'][rowNo].upper().split(fs)[3]
            except: continue
            if CPart == param:
                NPAR += 1
    param = 'DRAIN-FLOW'
    ...
    if param in paramGroups:
        for rowNo in range(len(QualAgDict['NAME'])):
            try: CPart = QualAgDict['PATH'][rowNo].upper().split(fs)[3]
            except: continue
            if CPart == param:
                NPAR += 1
    NOBS = nObs
