...
public class HDF5HydroGroup extends GroupProxy {
		...
		try {
			...
			for (Object meta : metadata) {
				...
				if (attr.getName().equals("Start time")) {
					startTime = TimeFactory.getInstance().createTime(((int[]) attr.getValue())[0]);
				}
			...
			if (hObject instanceof CompoundDS) {
				...
				int memberNameIndex = findIndexOfMemberName("chan_no", channelds);
				if (memberNameIndex == -1) {
					return null;
				}
				int channelLengthIndex = findIndexOfMemberName("length", channelds);
					...
					if (object instanceof int[]) {
						...
						for (int i = 0; i < channelArray.length; i++) {
							Pathname pathname = Pathname
									.createPathname(new String[] { "hydro", channelArray[i] + "_upstream", "flow",
											timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReference flowUpstream = new HDF5DataReference(file, "/hydro/data/channel flow",
									new int[] { i, 0 }, timeWindow.create(), timeInterval.create(timeInterval),
									pathname);
							references.add(flowUpstream);
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_downstream",
									"flow", timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReference flowDownstream = new HDF5DataReference(file, "/hydro/data/channel flow",
									new int[] { i, 1 }, timeWindow.create(), timeInterval.create(timeInterval),
									pathname);
							references.add(flowDownstream);
							// references for channel stage
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_upstream",
									"stage", timeWindow.toString(), timeInterval.toString(), modelRun });
							references.add(new HDF5DataReference(file, "/hydro/data/channel stage", new int[] { i, 0 },
									timeWindow.create(), timeInterval.create(timeInterval), pathname));
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_downstream",
									"stage", timeWindow.toString(), timeInterval.toString(), modelRun });
							references.add(new HDF5DataReference(file, "/hydro/data/channel stage", new int[] { i, 1 },
									timeWindow.create(), timeInterval.create(timeInterval), pathname));
							// references for channel area
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_upstream",
									"area", timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReference areaUpstream = new HDF5DataReference(file, "/hydro/data/channel area",
									new int[] { i, 0 }, timeWindow.create(), timeInterval.create(timeInterval),
									pathname);
							references.add(areaUpstream);
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_downstream",
									"area", timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReference areaDownstream = new HDF5DataReference(file, "/hydro/data/channel area",
									new int[] { i, 1 }, timeWindow.create(), timeInterval.create(timeInterval),
									pathname);
							references.add(areaDownstream);
							// references for channel average area
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "", "avg_area",
									timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReference avgArea = new HDF5DataReference(file, "/hydro/data/channel avg area",
									new int[] { i, 0 }, timeWindow.create(), timeInterval.create(timeInterval),
									pathname);
							references.add(avgArea);
							// references for channel water volume
							if (channelLengthIndex != -1) {
								pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "",
										"volume", timeWindow.toString(), timeInterval.toString(), modelRun });
								DataReferenceScalarMathProxy volume = new DataReferenceScalarMathProxy(avgArea,
										channelLengthArray[i], DataReferenceMath.MUL, DataReferenceMath.FIRST_FIRST);
							...
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_upstream",
									"velocity", timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReferenceVectorMathProxy refvmath = new DataReferenceVectorMathProxy(flowUpstream,
									areaUpstream, DataReferenceMath.DIV);
							refvmath.setPathname(pathname);
							refvmath.setUnits("FT/S");
							references.add(refvmath);
							pathname = Pathname.createPathname(new String[] { "hydro", channelArray[i] + "_downstream",
									"velocity", timeWindow.toString(), timeInterval.toString(), modelRun });
							refvmath = new DataReferenceVectorMathProxy(flowDownstream, areaDownstream,
			...
			HObject reservoirConnectionObject = h5file.get("/hydro/input/reservoir_connection");
			...
			if (reservoirObject instanceof CompoundDS) {
				...
				int memberNameIndex = findIndexOfMemberName("name", reservoirIds);
				if (memberNameIndex == -1) {
					return null;
				}
				...
				int nodeNumberIndex = findIndexOfMemberName("node", reservoirConnectionDS);
				//
				CompoundDS gateDS = (CompoundDS) gateObject;
				gateDS.getData();

				Object reservoirData = reservoirIds.getData();
				Object reservoirConnectionData = reservoirConnectionDS.getData();
				...
				if (reservoirData instanceof List) {
					...
					if (object instanceof String[]) {
						...
						for (int j = 0; j < reservoirNames.length; j++) {
							...
							Reservoir r = new Reservoir(reservoirNames[j]);
						...
						for (int k = 0; k < fromObj.length; k++) {
							if (fromObj[k].equalsIgnoreCase("reservoir")) {
								logger.debug("For reservoir: " + fromObjName[k] + " adding gate: " + gateNames[k]
										+ " to node: " + to_node[k]);
								fromObjName[k] = fromObjName[k].toUpperCase();
								reservoirMap.get(fromObjName[k]).addGateNode(to_node[k]);
							}
						}
						...
						for (int i = 0; i < reservoirNames.length; i++) {
							...
							Pathname pathname = Pathname.createPathname(new String[] { "hydro", reservoirNames[i],
									"stage", timeWindow.toString(), timeInterval.toString(), modelRun });
							...
							HDF5DataReference reservoirStage = new HDF5DataReference(file,
									"/hydro/data/reservoir height", new int[] { i, 0 }, timeWindow, timeInterval,
									pathname);
							references.add(reservoirStage);
							// references for reservoir volume
							pathname = Pathname.createPathname(new String[] { "hydro", reservoirNames[i], "volume",
									timeWindow.toString(), timeInterval.toString(), modelRun });
							DataReferenceScalarMathProxy reservoirDepth = new DataReferenceScalarMathProxy(
									reservoirStage, reservoirBottomElevations[i], DataReferenceMath.SUB,
							...
							DataReferenceScalarMathProxy reservoirVolume = new DataReferenceScalarMathProxy(
									reservoirDepth, reservoirAreas[i] * 1e06, DataReferenceMath.MUL,
									DataReferenceMath.FIRST_FIRST);
							...
							Reservoir reservoir = reservoirMap.get(reservoirNames[i]);

							Integer[] nodeArray = reservoir.getNodes();
							for (int j = 0; j < nodeArray.length; j++) {
								pathname = Pathname
										.createPathname(new String[] { "hydro", reservoirNames[i] + "@" + nodeArray[j],
												"flow", timeWindow.toString(), timeInterval.toString(), modelRun });
								references.add(new HDF5DataReference(file, "/hydro/data/reservoir flow",
										new int[] { reservoirIndexInData++, 0 }, timeWindow, timeInterval, pathname));
							}
							...
							for (int j = 0; j < gateNodeArray.length; j++) {
								pathname = Pathname.createPathname(
										new String[] { "hydro", reservoirNames[i] + "@" + gateNodeArray[j], "flow",
												timeWindow.toString(), timeInterval.toString(), modelRun });
								references.add(new HDF5DataReference(file, "/hydro/data/reservoir flow",
										new int[] { reservoirIndexInData++, 0 }, timeWindow, timeInterval, pathname));
							}
	...
	private void adjustBottomElevationsForStage() {
		// if channel get elevation at node position
		double bottomElevation = 0.0;
		if (path.contains("channel stage")) {
//			bottomElevation = getBottomElevation(h5file, locationNumber, nodePosition);
		}
	}
