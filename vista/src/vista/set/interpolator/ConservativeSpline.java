package vista.set.interpolator;
...
public class ConservativeSpline {
	...
	private void intone(double v) throws Exception {
		...
		if (indx >= (n - 1))
			indx = 0;
		if (v < x[0] || v > x[n - 1])
			throw new Exception("New abscissa values out of bounds (extrapolation)");
		if (v >= x[indx]) {
			if (v < x[indx + 1])
				return;
			l = (n - 1);
	...
	public static void main(String[] argv) {
		...
		try {
			PrintWriter outfile = new PrintWriter(
					new BufferedWriter(new FileWriter("d:/delta/tide/models/ConserveSpline/out2.txt")));
			for (int i = 0; i < 276; i++) {
		...
		double[] realpt = { 0., 0., 0. };
		double[] imagpt = { 0., 0., 0. };
		// RootFinding.DZPORC(ndeg, coef, realpt, imagpt); // -- No longer available to
		// link to ! is this even needed?
		for (int j = 0; j < 3; j++) {
