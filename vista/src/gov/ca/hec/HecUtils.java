...
public class HecUtils {
	...
	public static DataContainer getMatching(HecDss dss, String pattern) throws Exception {
		...
		if (matches.size() >= 1) {
			...
			System.out.println("No match for: " + pattern + ", " + matches);
			return null;
		}
	}
	/**
	...
	public static double calculateNashSutcliffeEfficiency(TimeSeriesContainer model, TimeSeriesContainer obs) throws HecMathException{
	    TimeSeriesMath modelt = new TimeSeriesMath(model);
	    TimeSeriesMath obst = new TimeSeriesMath(obs);
	...
	public static DataContainer average(DataContainer dataContainer, String intervalAsString) throws HecMathException {
	    return new TimeSeriesMath((TimeSeriesContainer) dataContainer).transformTimeSeries(intervalAsString, null, "AVE").getData();
	}
